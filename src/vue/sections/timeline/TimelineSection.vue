    ...
    <ul class="timeline">
      ...
      <li v-for="item in orderedItems" class="timeline-item">
        ...
        <div
          ...
          :class="{
            'timeline-item-logo-bg-primary': !(
              item['place'] && item['place']['logoUrl']
            ),
          }"
        >
        ...
        <div class="timeline-item-content">
          ...
          <div class="timeline-item-content-header">
            ...
            <div class="flex-column-title">
              ...
              <h6 class="timeline-item-subtitle text-light-6 text-3">
                ...
                <span>{{
                  item["place"] ? item["place"]["locales"]["name"] : ""
                }}</span>
              </h6>
            </div>
          ...
          <div class="timeline-item-content-body mt-3">
            <p
              class="text-3 text-normal mb-2 mb-md-3 mb-lg-4"
...
const _formatItemDate = (item) => {
  if (!item?.period?.length) return [];

  const [fromDate, toDate] = item.period;
  const langId = language.getSelectedLanguage()?.id || "en";
  const from = utils.localizeDate(fromDate, langId);

  let label;

  if (toDate) {
    if (typeof toDate === "string" && toDate.toLowerCase() === "present") {
      label = `${from} <span class="me-1 ms-1">➔</span> Present`;
    } else {
      const to = utils.localizeDate(toDate, langId);
